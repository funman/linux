 ...
 *
 *  2008-09-07: Arthur Tang <tangjingbiao@gmail.com>
 *      - port to 2.6.21.x
 *      - update to work with helper API changes and work with SMP
 *
 *
 */
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("ip_nat_mms");

static unsigned int nf_nat_mms(struct sk_buff **pskb,
                          enum ip_conntrack_info ctinfo,
                          struct nf_conntrack_expect *expect,
                          struct nf_ct_mms_expect *ct_mms_info)
{
        __be32 newip;
        ...
        DEBUGP("nf_nat_mms: info in %u, proto %s\n",
               ntohl(tcph->seq), 
	       mms_proto == IPPROTO_UDP ? "UDP": mms_proto == IPPROTO_TCP ? "TCP":proto_string);
        ...
        for (port = ct_mms_info->port; port != 0; port++) {
                ...
                if (nf_conntrack_expect_related(expect) == 0) {
                        DEBUGP("nf_nat_mms: using port %d\n", port);
                        break;
                }
        }
        ...
        DEBUGP("nf_nat_mms: zero_padding = %u\n", zero_padding);
	...
	{
		DEBUGP("nf_nat_mms: mangle error?\n");
		nf_conntrack_unexpect_related(expect);
		return NF_DROP;
	}
...
static void nf_nat_mms_fini(void)
{
	rcu_assign_pointer(nf_nat_mms_hook, NULL);
	synchronize_rcu();
}

static int __init nf_nat_mms_init(void)
{
	BUG_ON(rcu_dereference(nf_nat_mms_hook));
	rcu_assign_pointer(nf_nat_mms_hook, nf_nat_mms);
	return 0;
}

module_init(nf_nat_mms_init);
module_exit(nf_nat_mms_fini);
