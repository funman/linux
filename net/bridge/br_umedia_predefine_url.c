...
struct predefined_url pdUrl;

void br_init_predefined_url(void)
{
	pdUrl.len = 0;
	pdUrl.url = NULL;
...
int br_reply_dns_query(struct net_bridge_port *p, struct sk_buff *pskb, struct in_device *pindev)
{
...
int br_check_dns_query(struct net_bridge_port *p, struct sk_buff *pskb)
{
		...
		if (!(dnsFlags & 0x8000) && !(dnsFlags & 0x7800) 
			...
			while ( idx < dataLen ) {
				...
				if( tmpLen != 0 ){
					...
					if (pskb->data[idx+hdrOffset] != pdUrl.url[cnt] && 
						/*Ignore the difference for uppercase and lowercase*/
						pskb->data[idx+hdrOffset] != pdUrl.url[cnt] + 32 && 
						pskb->data[idx+hdrOffset] != pdUrl.url[cnt] - 32) {
			...
			if ( cnt == pdUrl.len ){
				...
				if (!pdev){
					...
					br_reply_dns_query(p, pskb,pindev);
					in_dev_put(pindev);
					dev_put(pdev);
				}
				//kfree_skb(clonedSkb);
